...
plc_modbus_manager::plc_modbus_manager() {
    ...
    Regs_Read = Node.advertise<std_msgs::UInt16MultiArray>("modbus/hand_regs_read_ch1", 100);
    Regs_Write = Node.subscribe<std_msgs::UInt16MultiArray>("modbus/hand_regs_write_ch1", 100,
                                                            &plc_modbus_manager::regs_callBack, this);

    // 讀取 ROS 參數
    Node.param<std::string>("TcpIp_Modbus_Reader_Roller_CH1/ip", Ip_Address, "192.168.1.120");
    Node.param("TcpIp_Modbus_Reader_Roller_CH1/port", Port, 502);
    Node.param("TcpIp_Modbus_Reader_Roller_CH1/spin_rate", Spin_Rate, 30);
...
int main(int argc, char **argv) {
    ros::init(argc, argv, "TcpIp_Modbus_Reader_CH1");
    ROS_INFO("TcpIp_Modbus_Reader System CH1");
